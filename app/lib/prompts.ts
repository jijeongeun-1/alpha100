...
유형 B — 목적·배경·개요 (수행목적, 과업 배경, 추진 필요성, 용역 목적, 과업개요, 완료내용 등)
  판별: 목적/배경/필요성/현황/개요/완료내용 키워드가 섹션명에 포함되거나,
        짧은 서술을 요하는 단일 영역 섹션
  출처: 추출 체크리스트 ③④⑤ (개요·목적·배경·수행분야 전체)
  방법:
    - 수요기업 현황·문제점 2~3문장 (사전보고서 배경 섹션 기반)
    - 과업 배경·도입 필요성 2~3문장 (사전보고서 필요성 섹션 기반)
    - 주요 과업 내용 요약 2~3문장 (수행 분야·주요과업내용 기반)
    - 완료 결과 1~2문장 (용역 완료 사실 언급)
    최소 분량: 200자 이상 (1줄 요약 금지). 각 단락은 완료형으로 재가공.
    ※ "용역 과업 개요", "과업 완료내용" 처럼 섹션명이 포괄적인 경우도 이 유형으로 처리하며,
       ③④⑤ 데이터를 모두 활용하여 충분한 분량으로 서술합니다.

유형 C — 수행계획·분야 (용역 내용, 수행 계획, 서비스 수행 분야 등)
  판별: 수행 구간·분야 목록이나 계획 서술이 예상되는 섹션
  출처: 추출 체크리스트 ④⑤ (배경·필요성 포함)
  방법: 분야별 구간을 가/나/다 목록으로 나열 + 각 구간마다 세부 서술 2~3문장.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
위에 제공된 [사후보고서 양식] 텍스트를 직접 읽고 문서 구조 전체를 스스로 파악합니다.

구조 파악 기준 (아래를 모두 단서로 활용합니다):
- 섹션 구분 기호: 숫자(1. 2.), ■, □, 제X장, 가나다, 굵은 소제목 등 형식에 무관하게 인식
- 표 구조: 공백 정렬(pdftotext -layout)로 표현된 열 정렬, 행 반복 패턴을 보고 표로 인식
  예) "용역 업체    대표자" → 2열 표 행 / "항목명    값    항목명    값" → 4열 표 행
- 섹션 경계: 빈 줄, 들여쓰기 변화, 반복 패턴 단절 등을 섹션 구분 단서로 활용
- 빈 영역: 양식에서 비어있는 큰 영역(실제로 작성해야 할 공간)을 해당 섹션의 content 작성 대상으로 인식
이러한 구조 단서를 통해 "작성해야 할 항목"을 빠짐없이 식별하여 templateSections를 구성합니다.

작성 규칙:
1. 양식의 모든 작성 항목을 양식에 등장하는 순서대로 templateSections에 포함합니다.
2. 각 섹션의 title은 양식에 표기된 항목명을 원문 그대로 사용합니다.
3. 표 형식 항목은 양식 원문의 행 항목(열 구성 포함)을 빠짐없이 재현합니다.
4. 표지·기본정보표(보고서 제목, 수요기업·수행기관 정보 등)는 항상 첫 번째 섹션으로 작성합니다.
5. 혼합 형식 항목(표+서술, 목록+설명)은 두 파트를 모두 작성합니다.
6. 단계별 서술 항목은 각 단계(가/나/다)마다 2~3문장 이상 작성합니다.
7. 결과물 항목은 납품물별로 특징·방법·의미를 구체적으로 서술합니다.
8. 사전보고서와 산출물 파일의 내용을 바탕으로 분량과 서술 깊이를 충분히 채웁니다.
   서술형 섹션(유형 B·C·F)은 최소 200자 이상 작성합니다. 1~2줄 짧은 요약으로 마치지 않습니다.
   사전보고서에 과업 배경·필요성·주요과업내용 등 세부 정보가 있으면 그 내용을 적극 반영합니다.
9. 하위 항목(<소제목>, (1)(2)(3), 가나다 소항목 등)은 별도 섹션으로 분리하지 않고 상위 섹션 content 내부에 HTML로 작성합니다.
