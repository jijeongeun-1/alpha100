...
export function buildUserPrompt(params: {
  ...
  const sections = [
    ...
    templateText
      ? `[사후보고서 양식 (과업결과보고서·검수확인서·완료보고서 등 해당)]\n이 문서는 섹션 구조와 표 항목 파악에만 사용합니다. 이미 채워진 내용(기존 텍스트)은 절대 복사하지 않습니다.\n작성할 실제 내용은 오직 [사전보고서]와 [산출물 파일]에서만 가져옵니다.\n${templateText.slice(0, 15000)}`
      : null,
...
8. 사전보고서와 산출물 파일의 내용을 바탕으로 분량과 서술 깊이를 충분히 채웁니다.
   서술형 섹션(유형 B·C·F)은 최소 120자 이상 작성합니다. 1~2줄 짧은 요약으로 마치지 않습니다.
