...
유형 E — 수행과정·결과 (수행 절차, 과정, 결과물, 납품물, 산출물 등)
  ...
  형식: 반드시 표 형태로 작성합니다. 열 구성: 구분(단계명) | 수행 내용 | 결과물 이미지
        이미지 placeholder는 "결과물 이미지" 열 안에 배치합니다.
  방법:
    ① 산출물 파일에서 화면명·기능명·섹션 제목·페이지 제목 등 식별 가능한 텍스트를 먼저 파악합니다.
    ② 사전보고서 ⑤의 수행 구간(단계)마다 표 행 1개씩 작성합니다.
    ③ "수행 내용" 열: 해당 구간에서 수행한 작업을 2~3문장으로 서술합니다.
    ④ "결과물 이미지" 열: 산출물 파일에서 파악한 화면·결과물 중 그 구간에 해당하는 것을 지정합니다.
       placeholder 텍스트에 어떤 이미지가 필요한지 구체적으로 명시합니다.
       예) [이미지 첨부 필요: 메인 홈 화면 UI 스크린샷]
           [이미지 첨부 필요: 상품 목록 페이지 디자인 결과물]
           [이미지 첨부 필요: 사전조사 자료 또는 벤치마킹 분석 화면]
  주의: 산출물 PDF가 이미지 위주라 텍스트 빈약할 수 있음.
        이 경우 파일명·목차·캡션 등 추출된 텍스트로 내용을 추론하여 구체적 설명을 작성합니다.
        URL·계정 정보가 있으면 표 아래에 별도 표로 추가합니다.
...
■ 수행과정·결과 섹션 (유형 E) — 표 형태 고정
구분(단계명) | 수행 내용 | 결과물 이미지 3열 표로 작성합니다.
"결과물 이미지" 열 안에 이미지 placeholder를 배치하며, 어떤 이미지인지 구체적으로 명시합니다.
출력 예:
<table style=\"width:100%;border-collapse:collapse;font-size:13px\"><tr><td style=\"border:1px solid #555;padding:5px 8px;background:#f0f0f0;font-weight:bold;width:20%\">구분</td><td style=\"border:1px solid #555;padding:5px 8px;background:#f0f0f0;font-weight:bold;width:45%\">수행 내용</td><td style=\"border:1px solid #555;padding:5px 8px;background:#f0f0f0;font-weight:bold;width:35%\">결과물 이미지</td></tr><tr><td style=\"border:1px solid #555;padding:5px 8px\">1. 사전조사</td><td style=\"border:1px solid #555;padding:5px 8px\">수요기업의 현황과 유사 서비스를 조사·분석하였습니다. 벤치마킹 자료를 토대로 방향성을 설정하였습니다.</td><td style=\"border:1px solid #555;padding:5px 8px\"><div style=\"border:2px dashed #bbb;padding:20px;text-align:center;color:#aaa;font-size:12px\">[이미지 첨부 필요: 벤치마킹 분석 자료 화면]</div></td></tr><tr><td style=\"border:1px solid #555;padding:5px 8px\">2. UI 설계</td><td style=\"border:1px solid #555;padding:5px 8px\">화면 구조와 사용자 흐름을 설계하였습니다. 주요 기능별 와이어프레임을 작성하였습니다.</td><td style=\"border:1px solid #555;padding:5px 8px\"><div style=\"border:2px dashed #bbb;padding:20px;text-align:center;color:#aaa;font-size:12px\">[이미지 첨부 필요: 메인 화면 와이어프레임 또는 UI 설계 결과물]</div></td></tr></table>
