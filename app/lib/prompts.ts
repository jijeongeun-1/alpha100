...
유형 E — 수행과정·결과 (수행 절차, 과정, 결과물, 납품물, 산출물 등)
  ...
  형식 (필수, 절대 생략 불가):
    - 반드시 3열 표로 작성합니다: 구분(단계명) | 수행 내용 | 결과물 이미지
    - 2열 표(구분|내용)로 작성하지 않습니다.
    - "결과물 이미지" 열은 생략할 수 없으며, 모든 행에 이미지 placeholder를 포함합니다.
    - 수행과정과 수행결과가 별도 섹션으로 분리된 경우 둘 다 동일한 3열 표 형식을 적용합니다.
  방법:
    ...
    ④ "결과물 이미지" 열: 반드시 이미지 placeholder를 삽입하고, 어떤 이미지인지 구체적으로 명시합니다.
       산출물 파일에서 확인된 화면명·결과물명을 사용합니다.
...
■ 수행과정·결과 섹션 (유형 E) — 3열 표 필수, 이미지 열 생략 불가
구분(단계명) | 수행 내용 | 결과물 이미지 — 반드시 3열로 작성합니다.
2열(구분|내용)로 줄이거나 이미지 열을 생략하면 안 됩니다.
모든 행의 "결과물 이미지" 열에 해당 단계의 결과물을 구체적으로 명시한 placeholder를 삽입합니다.
출력 예:
